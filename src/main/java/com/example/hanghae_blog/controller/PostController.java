...
public class PostController {
	...
	// 4. 선택한 게시글 수정
	@PutMapping("/post/{id}")
	public PostResponseDto update(@RequestBody PostRequestDto requestDto, HttpServletRequest request) {
		return postService.update(requestDto, request);
	}
	// 5. 선택한 게시글 삭제
	@DeleteMapping("/post/{id}")
	public DeleteReponseDto delete(@RequestBody HttpServletRequest request) {
